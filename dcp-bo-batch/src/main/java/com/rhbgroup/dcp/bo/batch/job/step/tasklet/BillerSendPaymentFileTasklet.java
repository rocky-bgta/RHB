...
@Component
@Lazy
public class BillerSendPaymentFileTasklet implements Tasklet {
	
    @Autowired
    @Qualifier("BillPaymentConfigOutboundQueue")
    private Queue<BillerPaymentOutboundConfig> queue ;
	
    static final Logger logger = Logger.getLogger(BillerSendPaymentFileTasklet.class);

    @Autowired
    JMSConfig billerSendPaymentFileJMSConfig;
	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) {
		try {
			...
			JMSUtils.sendMessageToJMS(jsonStr, billerSendPaymentFileJMSConfig);

...
@Component
@Lazy
public class PrepaidReloadExtractionCASAStepBuilder extends BaseStepBuilder{
    static final Logger logger = Logger.getLogger(PrepaidReloadExtractionCASAStepBuilder.class);
    private static final String STEPNAME = "PrepaidReloadExtractionCASA";
    @Autowired
    ...
    private PagingQueryProvider createQueryProvider(StepExecution stepExecution) {
        ...
        queryProvider.setWhereClause("WHERE payment_type='CASA' AND (txn_time>='"+startDate+"' AND txn_time<'"+endDate+"')");
    ...
    public FlatFileItemWriter<PrepaidReloadExtractionOut> prepaidReloadJobWriter(@Value("#{stepExecution}") StepExecution stepExecution) throws ParseException {
        ...
        String targetFileNewName=configProperties.getNamecasa().replace("{#date}",  DateUtils.formatDateString(DateUtils.getDateFromString(batchSystemDatStr,DEFAULT_DATE_FORMAT), configProperties.getNamedateformat()));

...
public interface ApprovalRepository extends JpaRepository<Approval, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE Approval x SET x.status = :status, x.reason = :rejectReason WHERE x.id = :id ")
    Integer updateStatusById(@Param("status") String status, @Param("id") int id, @Param("rejectReason") String rejectReason);

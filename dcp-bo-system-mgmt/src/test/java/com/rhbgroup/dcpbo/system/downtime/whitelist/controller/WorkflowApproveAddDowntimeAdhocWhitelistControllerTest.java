...
@SpringBootTest(classes = {WorkflowApproveAddDowntimeAdhocWhitelistControllerTest.class, WorkflowDowntimeAdhocWhitelistController.class})
@EnableWebMvc
public class WorkflowApproveAddDowntimeAdhocWhitelistControllerTest {
	...
	public void addDowntimeAdhocWhitelistApprovalSuccessTest() throws Exception {
		...
		int approvalId = 123;
		String url = "/bo//workflow/downtime/whitelist/approval/" + approvalId;
		...
		when(workflowDowntimeAdhocWhitelistServiceMock.approveAddDowntimeWhitelist(Mockito.anyInt(), Mockito.anyObject(), Mockito.anyString())).thenReturn(adhocApproval);

		mockMvc.perform(MockMvcRequestBuilders.post(url)
				.header("userId", userId)
				.contentType(MediaType.APPLICATION_JSON).content(asJsonString(approveAddDowntimeAdhocWhitelistRequest)))
	...
	@Test
	public void addDowntimeAdhocWhitelistApprovalDuplicateUserFailTest() throws Exception {
		...
		when(workflowDowntimeAdhocWhitelistServiceMock.approveAddDowntimeWhitelist(Mockito.anyInt(), Mockito.anyObject(), Mockito.anyString())).thenThrow(CommonException.class);
